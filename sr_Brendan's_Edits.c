...
void A_output(struct msg message)
{
    ...
    //if window does not pass the end and is inside base to base+window size
    boolean standardBound = (send_base + WINDOWSIZE )< 2*WINDOWSIZE && nextseqnum <= send_base  && nextseqnum > (send_base + WINDOWSIZE);
    // if full window would wrap around
    boolean wraparoundBound = (send_base + WINDOWSIZE >= 2 * WINDOWSIZE) && (nextseqnum <= send_base && nextseqnum > 2*WINDOWSIZE)  || (nextseqnum >= 0 && nextseqnum > (send_base + WINDOWSIZE)%(2 * WINDOWSIZE)); 
    if(standardBound || wraparoundBound)
    {
    ...
    set to RTT (which is defined in the file). 
         */
        memcpy(&winbuf[winfront], &sendPacket, sizeof(struct pkt));

        if(pktnum == 0)
        {
            starttimer(A,nextseqnum, RTT);
        }
        //increase pktnum and move front of buffer
        nextseqnum = (nextseqnum + 1) % (2*WINDOWSIZE) ;
        pktnum = pktnum + 1;
...
{
    /* First, check if the packet is corrupted. If it is not, delete the ack’ed
packets from window buffer, advance the window base pointer, and stop the timer. If the
message buffer is not empty (i.e., there are application messages waiting to be sent), create a
new packet and send it. */
    if(!CheckCorrupted(packet))
    {
        //scan from winrear to winfront 
        if (winfront > winrear)
        {
            
            for(int i = winrear;i < winfront; i++ )
            {
              if(winbuf[i].seqnum == packet.seqnum ) 
              {
                  winbuf[i].acknum = TRUE;
                  stoptimer(A, winbuf[i].seqnum );
                  break;
              }
            }
        }else
        {
            for(int i = winrear;i < WINDOWSIZE ; i++ )
            {
              if(winbuf[i].seqnum == packet.seqnum ) 
              {
                  winbuf[i].acknum = TRUE;
        ...
        //move forward winrear shrinking window
        while(winrear != winfront)
        {
            if(winbuf[winrear].acknum = TRUE){
        ...
        int tempwinrear = winrear;
        while(tempwinrear != winfront)
        {
            if(!( winbuf[tempwinrear].acknum = TRUE))
            {
                starttimer(A,winbuf[tempwinrear].seqnum, RTT);
                break;
            }
            tempwinrear = (tempwinrear +1 ) % WINDOWSIZE;
        }
        // if buffer is not empty take one from the rear and send it
        if(msgnum > 0 && pktnum < WINDOWSIZE)
        {
            struct pkt sendPacket;
            sendPacket.seqnum = nextseqnum;
            sendPacket.acknum = NOTUSED;    
            memcpy(sendPacket.payload , &buffer[bufrear], PAYLOADSIZE);
            sendPacket.checksum = ComputeChecksum(sendPacket);
            //pass to layer 3
            tolayer3(A,sendPacket);
            /*. Copy the packet to the buffer defined by
...
void A_timerinterrupt(int seqnum)
{
    ...
    for( int i = 0; i < pktnum; i++)
    {
       ...
       {
           tolayer3(A,winbuf[winrear + i]);
           break;
       }
    }
    starttimer(A,seqnum,RTT);
